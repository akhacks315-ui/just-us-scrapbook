...
function renderTimeline(){ $("#timeline").innerHTML=timeline.map(x=>`<article class="timeline-item reveal"><div class="timeline-date">${x.date}</div><h3>${x.title}</h3><p>${x.text}</p></article>`).join("") }
...
function renderThings(){ $("#things").innerHTML=things.map((x,i)=>`<div class="thing reveal"><span>0${i+1}</span><p>${x}</p></div>`).join("") }
